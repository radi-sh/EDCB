...
■チャンネル切換処理を2回行う■
　チャンネル切換処理を強制的に2回行います。
　特定のチューナーとBonDriverの組み合わせで、チャンネル切換に失敗したり
　Dropが多発したりする症状がある場合に試してみてください。
　BonCtrl.iniにBonDriverのファイル名（拡張子含む）のセクションを追加し、
　以下のものを追加することで設定可能。
　　TwiceSetCh：1で機能を有効、0で機能を無効
　　TwiceSetChWait：2回目のチャンネル切換を行うまでのWait(msec単位)
　　（デフォルト:500、1000以上は設定できません）
